...
class AmonLib {
  ...
  constructor(opts) {
    ...
    this.coinsInstances = Object.keys(Coins).reduce((acc, coinCode) => {
      if (typeof Coins[coinCode] === 'object') {
        Object.keys(Coins[coinCode]).reduce((_, tokenCode) => {
          acc[`${coinCode}_${tokenCode}`] = new Coins[coinCode][tokenCode](opts);
        }, {});
      } else {
        acc[coinCode] = new Coins[coinCode](opts);
      }
      return acc;
    }, {});
