...
class AmonLib {
  ...
  constructor(opts) {
    ...
    this.coinsInstances = Object.keys(Coins).reduce((acc, code) => {
      console.log('Coins[code]', Coins[code] )
      Object.keys(Coins[coinCode]).reduce((acc, coinCode) => {

      console.log('coinCode', coinCode )
      acc[coinCode] = new Coins[code][coinCode](this.opts);
      return acc;
      },{})
    }, {});
    console.log('this.coinsInstances', this.coinsInstances )
